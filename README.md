# MAIN TAINER
-Ramdhany Fataruba

# DOKUMEN/HOW TO 
-Hidayat Abidi 
-Muhammad Abdul Hafiedh Rettob

# Nama-Nama Kontributor
-Taufiqqurahman Andar
-Abubakar Ismail
-Hidayat Abidi
-Muhammad Abdul Hafiedh Rettob
-Ramdhany Fataruba



# Deskripsi Projek
SpaceAssault adalah game arcade bertema pertempuran luar angkasa di mana pemain mengendalikan pesawat tempur yang berada di bagian bawah layar untuk melawan musuh yang datang dari atas, dengan latar belakang angkasa gelap yang dipenuhi bintang dan awan sehingga suasana permainan terasa seperti pertarungan di luar angkasa. Di dalam game ini ditampilkan beberapa informasi penting seperti score, level, jumlah nyawa (lives), serta bom (B-2) yang dapat digunakan pemain, di mana semakin banyak musuh yang berhasil dihancurkan maka skor dan level akan meningkat dan tingkat kesulitan permainan juga ikut bertambah. Tujuan utama dari SpaceAssault adalah bertahan selama mungkin dengan cara menghancurkan musuh dan menghindari serangan mereka, sehingga game ini menuntut refleks yang cepat, fokus, serta strategi sederhana agar pemain tidak cepat kehabisan nyawa.

# Yang di butuhkan oleh projek ini
Game SpaceAssault dibuat menggunakan bahasa pemrograman Python dengan bantuan library Pygame sebagai inti untuk mengatur tampilan game, gerakan, suara, dan kontrol pemain, lalu didukung juga oleh modul bawaan seperti random untuk membuat musuh muncul secara acak, json untuk menyimpan skor, serta sys, os, dan math untuk kebutuhan sistem dan perhitungan sederhana, jadi walaupun kelihatannya kompleks, sebenarnya semua masih berbasis Python dengan library pendukung saja.

# How to use 
SpaceAssault jalankan game lalu gunakan tombol **W/A/S/D atau panah** untuk menggerakkan pesawat, tekan **SPACE** untuk menembak musuh, dan tekan **B** untuk menggunakan bom B-2 saat keadaan darurat. Hindari serangan musuh, hancurkan sebanyak mungkin musuh untuk menaikkan score dan level, dan manfaatkan power-up yang muncul agar bisa bertahan lebih lama.
